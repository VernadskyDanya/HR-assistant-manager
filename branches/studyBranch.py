...
def study(message_chat_id, bot):
    ...
    button2 = types.InlineKeyboardButton(text="Я знаю, чему обучиться/обучить сотрудника", callback_data="iKnow")
    ...
    markup.row(button4)
    markup.row(button5)
    bot.send_message(message_chat_id, "📘 Выберите вопрос по обучению:", reply_markup=markup)
...
def iKnow(message_chat_id, bot):
    ...
    bot.send_message(message_chat_id, "Если вы выбрали для себя или своего сотрудника <b>конкретную "
                                      "программу</b>, направьте данную информацию своему HR-партнеру для"
                                      " обсуждения возможностей бюджета и запуска процедуры",
                     parse_mode = 'HTML', reply_markup=markup)
...
def restriction(message_chat_id, bot):
    ...
    bot.send_message(message_chat_id, "Дорогостоящее обучение <b>от 350 до 500 тыс</b> - "
                                      ...
                                      "директора дирекции, корпоративного университета и заместителя генерального "
                                      "директора блока организационных вопросов\n"
                                      "Дорогостоящее обучение <b>от 500 тыс</b> - требует тех же "
